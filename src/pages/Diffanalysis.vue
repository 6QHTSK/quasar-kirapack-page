    ...
    <div style="padding:10px" class="column q-gutter-sm">
        <div class="column q-gutter-sm">
            <div class="row q-gutter-lg">
                ...
                <q-space/>
                <q-btn :color="returncolor(diffid)" @click="clickonbutton()" :label="diffstr[diffid]" style="width:80px" v-show="!usingbestdori">
                </q-btn>
            </div>
        ...
        <div class="column q-gutter-sm">
            <q-btn color="primary" @click="analysis(false)" class="full-width" :readonly="onloading || showdetail" :loading="onloading" v-show="!usingbestdori && !showdetail">分析
                <template v-slot:loading>
                    <q-spinner-gears class="on-left" />
                    <span>计算中</span>
                </template>
            </q-btn>
            <q-btn color="primary" @click="getbestdorichart" :loading="loadingchart || onloading" class="full-width" v-show="usingbestdori && !showdetail">
                    获取
                    <template v-slot:loading>
                        <q-spinner-gears class="on-left"/>
    ...
    <div v-show="showdetail" class="q-gutter-sm row justify-evenly wrap items-start ">
      <div class="q-gutter-sm row wrap justify-evenly items-start">
        <div v-for="(subject,i) in itemSubject" :key="i" class="q-gutter-sm row fit wrap justify-center items-start ">
          <span style="width:100%;text-align:center" v-if="itemList[subject] && itemList[subject][0][2] != 0">{{itemLabel[i]}}</span>
...
export default {
  ...
  methods: {
    analysis (flag) {
      ...
      function procession (res) {
        if (res.status === 200 && res.data.result) {
          ...
          vm.itemList.basic = [ // 标题，数值前缀，数值，数值后缀，难度前缀，难度，难度后缀
            ...
            ['总物量', '', totalNote, '', '', '', ''],
            ['总敲击次数', '', totalHitNote, '', '', '', ''],
            ['击打音符占比', 'L.', fix(detail.leftPercent * 100, 2), '%', 'R.', fix(detail.leftPercent * 100, 2), '%'],
            ['主体部分谱面占比', '', fix(detail.activePercent * 100, 2), '%', '', '', ''],
            ['加权难度', 'LV.', fix(diff.totalDiff, 1), '', '', '', ''],
            ['蓝白物量难度', 'LV.', fix(diff.blueWhiteFunc, 0), '', '', '', '']
          ]
          vm.itemList.pressure = [
            ['整体每秒音符数', '', fix(detail.totalNPS, 2), '个/s', 'LV.', fix(diff.totalNPS, 1), ''],
            ['主体每秒音符数', '', fix(detail.activeNPS, 2), '个/s', 'LV.', fix(diff.activeNPS, 1), ''],
            ['整体每秒击打数', '', fix(detail.totalHPS, 2), '次/s', 'LV.', fix(diff.totalHPS, 1), ''],
            ['主体每秒击打数', '', fix(detail.activeHPS, 2), '次/s', 'LV.', fix(diff.activeHPS, 1), '']
          ]
          vm.itemList.flick = [
      ...
      var url
      if (flag) {
        url = 'https://testapi.ayachan.fun:17556/DiffAnalysis?id=' + this.bestdoriid
        if (this.bestdoriid < 500) {
          url = url + '&diff=' + this.diffid
        }
        this.$axios.get(url).then(res => { procession(res) })
      } else {
        url = 'https://testapi.ayachan.fun:17556/DiffAnalysis'
        try {
          var chart = JSON.parse(this.inputstr)
        } catch {
          vm.$q.notify({ message: 'JSON解析失败', type: 'Warning' })
          vm.onloading = false
          return false
        }
        var data = { diff: this.diffid, data: chart }
        this.$axios.post(url, data).then(res => { procession(res) }).catch(function (error) {
          vm.$q.notify(error)
          vm.onloading = false
        })
      }
    ...
    getbestdorichart () {
      ...
      this.$forceUpdate()
      if (this.bestdoriid > 500) {
        url = 'https://api.ayachan.fun/bdbasic?id=' + this.bestdoriid
      } else {
        url = 'https://api.ayachan.fun/bdofftobdfan?id=' + this.bestdoriid + '&diff=' + diffstr[this.diffid]
      }
      this.$axios.get(url).then(function (res) {
        if (res.status === 200) {
          ...
          if (vm.bestdoriid > 500 && res.data.result) {
            ...
            vm.$q.notify('谱面id输入错误')
            vm.loadingchart = false
          }
          vm.$forceUpdate()
        } else {
          vm.$q.notify('访问bestdori失败')
          vm.loadingchart = false
        }
